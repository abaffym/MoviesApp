...
class MainActivity : AppCompatActivity(), ExploreAdapter.OnMovieClickedListener {
    ...
    private fun setupTabBar() {
        ...
        tabLayout.onTabSelected(::navigateToTab)
    }
    ...
    private fun navigateToTab(position: Int) {
        when (position) {
            TAB_POSITION_EXPLORE -> supportFragmentManager.beginTransaction()
                    .replace(R.id.fragment_container, exploreFragment, FRAGMENT_TAG_EXPLORE)
                    .commit()
            TAB_POSITION_FAVORITE -> supportFragmentManager.beginTransaction()
